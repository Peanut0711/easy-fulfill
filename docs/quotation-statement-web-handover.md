...
2026-09-08 EASYFULFIL GUI 계산 규칙을 반영했다. 별도 웹 저장소의 구현 반영 여부는 별도로 확인해야 한다.
...
공급 단가는 할인 계산 도중 반올림하지 않고 마지막에 한 번 반올림한다. 최종 합계는 공급가액과 품목별 반올림 세액의 합이므로 할인 후 VAT 포함 단가 × 수량과 차이가 날 수 있다. 화면의 할인액은 할인 후 VAT 포함 단가 기준이며 이 반올림 차액을 포함하지 않는다. 네이버·쿠팡 주문 불러오기는 실제 결제금액을 보존하는 기존 계산을 유지한다.

예: 10,000원 상품 400개는 할인 전 4,000,000원이므로 10% 할인이다. 공급 단가는 8,182원, 공급가액은 3,272,800원, 세액은 327,280원, 최종 합계는 3,600,080원이다.
