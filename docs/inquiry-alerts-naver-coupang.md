...
- **예외 — 쿠팡 CS문의(`KC`)는 하루 1회만 재알림(🔁).** 자동 반품수거처럼 회사 도착까지
  판매자가 액션할 수 없는 건이 많아 매시간 알림이 소음이 되어, 「최근알림시각」의 날짜가
  오늘 이전일 때만 재알림한다(그날 이미 알렸으면 스킵 → 다음 근무일 1회). 신규 즉시 알림과
  자동 중단은 다른 채널과 동일.
...
- 알림 형태: **새 건 즉시 + 60분 주기 리마인더**. 단 **쿠팡 CS문의(KC)는 하루 1회 리마인더**.
