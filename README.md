...
3. 생성이 끝나면 HTML 편집 창이 열리고, 생성한 상품번호가 **쿠팡 상품 HTML 적용**의 네이버 상품번호에 자동 입력됩니다. 편집 창에서는 PC/모바일 미리보기, 항목 선택·삭제, 실행 취소, 저장(`Ctrl+S`)을 사용할 수 있습니다.
   - **여백 추가:** 미리보기에서 항목을 선택하고 **선택 영역 위/아래**, 간격(12·24·48px 또는 직접 입력)을 정한 뒤 **여백 추가**를 누릅니다. 여러 항목은 연속 선택해야 하며, 본문·인용구·이미지 묶음의 바깥에 여백이 들어갑니다. 기존 간격에 지정한 높이가 추가됩니다.
   - **여백 수정:** 점선으로 표시된 여백을 클릭하고 높이를 입력한 뒤 **높이 변경**을 누릅니다. **여백 삭제** 또는 `Delete`로 제거하고, `Ctrl+Z`로 추가·변경·삭제를 취소할 수 있습니다. 점선과 높이 안내는 편집 미리보기에만 표시되며 저장·복사하는 HTML에는 포함되지 않습니다.
4. 기존 쿠팡 상품에 적용할 때는 쿠팡 등록상품 ID를 입력한 뒤 **WING에 HTML 채우기**를 누릅니다. 저장된 `coupang-paste.html`이 있어야 하며, 기본 등록 → HTML 작성 전환과 HTML 입력까지만 자동 수행합니다. 저장은 열린 WING 화면에서 직접 진행합니다. 저장·검토요청 후 WING 창을 닫으면 작업 로그에 창 닫힘 감지가 기록되고, 작업 종료·종료 버튼 비활성화·입력 초기화가 자동으로 처리됩니다. 창을 닫지 않고 취소할 때만 **WING 작업 종료**를 누릅니다.
