...
from tracking.repository import (
    ...
    read_config_values_map as _read_config_values_map,
...
def _standalone_open_config_ws(gc):
    """기존 호출부 호환용: 실제 Sheet 생성·열기는 tracking.repository에 위임한다."""
...
CONFIG_KEY_AUTO_REFRESH_MIN = "tracking_auto_refresh_min"     # 자동 새로고침 간격(분). 0/빈=끔
CONFIG_KEY_LAST_AUTO_REFRESH = "tracking_last_auto_refresh"   # 마지막 자동조회 시각(YYYY-MM-DD HH:MM:SS)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._tracking_controller.initialize_runtime()
    ...
    def _tracking_table_col(self, source_col):
        return self._tracking_controller._tracking_table_col(source_col)

    def _evaluate_risk(self, status, where, done, ref, now_dt, management=""):
        return self._tracking_controller._evaluate_risk(status, where, done, ref, now_dt, management)

    def _on_tracking_list_poll(self):
        return self._tracking_controller._on_tracking_list_poll()

    def _on_tracking_context_menu(self, pos):
        return self._tracking_controller._on_tracking_context_menu(pos)

    def _load_tracking_list(self):
        return self._tracking_controller._load_tracking_list()

    def _on_tracking_list_finished(self, payload: dict):
        return self._tracking_controller._on_tracking_list_finished(payload)

    def _cleanup_tracking_list_thread(self):
        return self._tracking_controller._cleanup_tracking_list_thread()

    def _populate_tracking_table(self, *_args):
        return self._tracking_controller._populate_tracking_table(*_args)

    def _parse_event_dt(self, s):
        return self._tracking_controller._parse_event_dt(s)

    def _selected_tracking_regino(self):
        return self._tracking_controller._selected_tracking_regino()

    def _selected_tracking_reginos(self):
        return self._tracking_controller._selected_tracking_reginos()

    def _tracking_management_for_regino(self, regino):
        return self._tracking_controller._tracking_management_for_regino(regino)

    def _on_tracking_stop_selected_clicked(self):
        return self._tracking_controller._on_tracking_stop_selected_clicked()

    def _change_tracking_management(self, reginos, stop):
        return self._tracking_controller._change_tracking_management(reginos, stop)

    def _on_tracking_management_update_finished(self, payload):
        return self._tracking_controller._on_tracking_management_update_finished(payload)

    def _cleanup_tracking_management_update_thread(self):
        return self._tracking_controller._cleanup_tracking_management_update_thread()

    def _on_tracking_item_changed(self, item):
        return self._tracking_controller._on_tracking_item_changed(item)

    def _flush_tracking_notes(self):
        return self._tracking_controller._flush_tracking_notes()

    def _on_tracking_notes_update_finished(self, payload):
        return self._tracking_controller._on_tracking_notes_update_finished(payload)

    def _cleanup_tracking_notes_update_thread(self):
        return self._tracking_controller._cleanup_tracking_notes_update_thread()

    def _on_tracking_cell_double_clicked(self, row, col):
        return self._tracking_controller._on_tracking_cell_double_clicked(row, col)

    def _on_tracking_refresh_one_clicked(self):
        return self._tracking_controller._on_tracking_refresh_one_clicked()

    def _on_tracking_refresh_one_finished(self, payload: dict):
        return self._tracking_controller._on_tracking_refresh_one_finished(payload)
    ...
    def _enqueue_tracking_registration(self, records):
        return self._tracking_controller._enqueue_tracking_registration(records)

    def _flush_tracking_records(self):
        return self._tracking_controller._flush_tracking_records()

    def _on_tracking_upsert_finished(self, payload: dict):
        return self._tracking_controller._on_tracking_upsert_finished(payload)

    def _cleanup_tracking_op_thread(self):
        return self._tracking_controller._cleanup_tracking_op_thread()

    def _set_tracking_summary(self, text):
        return self._tracking_controller._set_tracking_summary(text)

    def on_refresh_tracking_clicked(self):
        return self._tracking_controller.on_refresh_tracking_clicked()

    def on_refresh_tracking_exceptions_clicked(self):
        return self._tracking_controller.on_refresh_tracking_exceptions_clicked()

    def _begin_tracking_refresh(self, scope):
        return self._tracking_controller._begin_tracking_refresh(scope)

    def _on_tracking_refresh_progress(self, done: int, total: int):
        return self._tracking_controller._on_tracking_refresh_progress(done, total)

    def _on_tracking_refresh_finished(self, payload: dict):
        return self._tracking_controller._on_tracking_refresh_finished(payload)

    def _cleanup_tracking_refresh_thread(self):
        return self._tracking_controller._cleanup_tracking_refresh_thread()

    def _auto_refresh_interval_min(self):
        return self._tracking_controller._auto_refresh_interval_min()

    def _reconfigure_auto_refresh_timer(self):
        return self._tracking_controller._reconfigure_auto_refresh_timer()

    def _on_tracking_auto_refresh_tick(self):
        return self._tracking_controller._on_tracking_auto_refresh_tick()

    def _on_tracking_auto_refresh_finished(self, payload: dict):
        return self._tracking_controller._on_tracking_auto_refresh_finished(payload)

    def _sync_auto_refresh_spinbox(self):
        return self._tracking_controller._sync_auto_refresh_spinbox()

    def _on_auto_refresh_interval_changed(self, value):
        return self._tracking_controller._on_auto_refresh_interval_changed(value)
    ...
    def setup_connections(self):
        ...
        self._tracking_controller.bind_ui()
    ...
    def _mount_detail_page_tab(self):
        ...
        self.pushButton_detail_clear_inputs.clicked.connect(self._on_detail_clear_inputs_clicked)
    ...
    def _on_detail_clear_inputs_clicked(self):
        self.lineEdit_detail_naver_create.clear()
        self.lineEdit_detail_naver_replace.clear()
        self.lineEdit_detail_vendor_inventory.clear()
    ...
    def _build_naver_orders_from_df(self, df, product_mapping):
        ...
        try:
            ...
            # 주문 정보 정리: DataFrame 변환 규칙은 화면과 독립된 orders.bulk에 둔다.
            print("\n[주문 정보 정리]")
            self.orders = build_naver_orders(
                df, required_columns, product_mapping, self._normalize_key_for_mapping
            )
    ...
    def process_gmarket_excel_file(self):
        ...
        try:
            ...
            # 주문 정보 정리와 수령인 기준 통합 규칙은 화면과 독립된 orders.bulk에 둔다.
            self.orders = build_gmarket_orders(df, required_columns)
            consolidated_orders = consolidate_gmarket_orders(self.orders)
    ...
    def export_invoice_excel(self):
        ...
        try:
            ...
            # 스토어별 orders -> 송장 행 변환은 화면과 독립된 orders.invoice에 둔다.
            invoice_data = build_invoice_rows(self.store_type, self.orders)
            # 데이터프레임 생성
            df_invoice = pd.DataFrame(invoice_data, columns=INVOICE_COLUMNS)
    ...
    def export_courier_receipt(self):
        return self._tracking_controller.export_courier_receipt()
